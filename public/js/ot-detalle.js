...
function renderOT() {
  ...
  <div class="otd">
    ...
    <div class="otd-header">
      <div class="otd-header-row">
        ...
        ${App.canEdit() ? `<button class="otd-icon-btn" id="btnEditarOT" aria-label="Editar">${EDIT_HDR_SVG}</button><button class="otd-icon-btn" id="btnEliminarOT" aria-label="Eliminar orden" style="color:#DC2626">${TRASH_SVG}</button>` : `<span style="width:36px"></span>`}
      </div>
      <div class="otd-meta">
  ...
  document.getElementById('btnEliminarOT')?.addEventListener('click', eliminarOrden);
  ...
  App.confirmarDoble(
    ...
    async () => {
      try {
        ...
        App.toast('Orden eliminada', 'success');
